-- 00237_fix_proposal_integrity_and_transaction.sql
--
-- Fixes three proposal integrity issues:
-- 1. Save proposal + sections + items + annual fixed costs atomically in one RPC.
-- 2. Preserve section discounts as first-class persisted data in the package save path.
-- 3. Recompute stale USD completion totals net of section-level discounts before
--    auto_record_proposal_revenue reads NEW.total_amount_usd.
...
BEGIN
  ...
  WITH section_subtotals AS (
    SELECT
      COALESCE(ps.discount_percent, 0) AS discount_percent,
      COALESCE(SUM(COALESCE(NULLIF(pi.total_usd, 0), COALESCE(pi.unit_price_usd, 0) * COALESCE(pi.quantity, 0))), 0) AS subtotal
    FROM proposal_sections ps
    LEFT JOIN proposal_items pi
      ON pi.section_id = ps.id
  ...
  SELECT COALESCE(ROUND(SUM(section_total), 2), 0)
  INTO v_computed_usd
  FROM (
    SELECT ROUND(subtotal - ROUND(subtotal * LEAST(GREATEST(discount_percent, 0), 100) / 100, 2), 2) AS section_total
    FROM section_subtotals
...
GRANT EXECUTE ON FUNCTION complete_proposal_with_rate(UUID, DECIMAL, DECIMAL) TO authenticated;

-- REVERT
-- DROP FUNCTION IF EXISTS fn_save_proposal_package(UUID, JSONB, JSONB, JSONB, JSONB);
-- To revert complete_proposal_with_rate, restore the previous guarded definition from 00230.
