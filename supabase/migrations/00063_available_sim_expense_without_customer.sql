-- Migration: 00063_available_sim_expense_without_customer
-- Description: Allow expense creation for available SIMs when customer_id is NULL.
-- Business rule: Boşta (available) SIM'ler için sadece gider aksın; gelir için customer_id zorunlu kalır.
...
BEGIN
  ...
  -- 1. EXPENSE (cost_price) — for both available and active (customer_id can be NULL for available)
  ...
  -- 2. INCOME (sale_price) — only for active, wholesale, and requires customer_id
  IF NEW.status::text = 'active' AND COALESCE(NEW.sale_price, 0) > 0 AND NEW.customer_id IS NOT NULL THEN
