-- Migration: 00154_dynamic_vat_triggers
-- Description: Phase 4 G6 — Replace hardcoded 20% VAT in finance triggers with
--   dynamic rates where available (subscriptions.vat_rate) and explicit
--   fallback format (20/100.0) for tables without vat_rate column.

-- ============================================================================
-- 1. fn_subscription_payment_to_finance — use subscription's vat_rate
-- ============================================================================
...
-- ============================================================================
-- 2. auto_record_proposal_revenue — explicit fallback (proposals has no vat_rate)
-- ============================================================================
...
DECLARE
  ...
  v_vat_rate DECIMAL(5,2) := 20;  -- Fallback until proposals.vat_rate exists
BEGIN
...
-- ============================================================================
-- 3. auto_record_work_order_revenue — explicit fallback (work_orders has no vat_rate)
-- ============================================================================
...
DECLARE
  ...
  v_expense_category_id UUID;
  v_vat_rate DECIMAL(5,2) := 20;  -- Fallback until work_orders.vat_rate exists
BEGIN
  IF NEW.status <> 'completed' OR OLD.status = 'completed' THEN
    RETURN NEW;
  END IF;
...
-- ============================================================================
-- 4. fn_sim_card_to_finance — explicit fallback (sim_cards has no vat_rate)
-- ============================================================================
...
DECLARE
  ...
  v_vat_rate     DECIMAL(5,2) := 20;  -- Fallback until sim_cards.vat_rate exists
BEGIN
