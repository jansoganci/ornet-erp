-- Migration: 00117_guard_bulk_update_subscription_prices
-- Description: Add a role guard to bulk_update_subscription_prices() so only
--   admin and accountant can execute it. Previously the SECURITY DEFINER function
--   was granted to all authenticated users with no internal role check, allowing
--   any field_worker to overwrite all subscription prices via the API.
--   The function body is otherwise identical to 00024.
...
BEGIN
  -- Role guard: only admin and accountant may bulk-update prices
  v_role := get_my_role();
  ...
  FOR i IN 0..(jsonb_array_length(p_updates) - 1) LOOP
    ...
    -- Get billing_frequency for payment amount logic
