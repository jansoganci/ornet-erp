-- 00236_fix_proposal_completion_exchange_rate.sql
--
-- Regression from 00212_tahsilat_core: auto_record_proposal_revenue was rewritten
-- without the completion_exchange_rate branch added in 00210. USD proposals
-- completed via complete_proposal_with_rate therefore ignored the user-confirmed
-- rate and always used exchange_rates lookup.
--
-- 1. Restore user-confirmed rate preference in the trigger.
-- 2. Backfill income + COGS rows where stored rate differs from completion_exchange_rate.
...
BEGIN
  ...
  -- Prefer user-confirmed rate from complete_proposal_with_rate RPC (00210).
...
-- Backfill ledger rows written with auto-lookup rate instead of user-confirmed rate.
UPDATE financial_transactions ft
SET
  exchange_rate = p.completion_exchange_rate,
  amount_try    = ROUND(ft.amount_original * p.completion_exchange_rate, 2),
  output_vat    = ROUND(
    ROUND(ft.amount_original * p.completion_exchange_rate, 2) * COALESCE(p.vat_rate, 20) / 100,
    2
  ),
  cogs_try      = CASE
    WHEN ft.cogs_try IS NOT NULL AND ft.cogs_try > 0 THEN (
      SELECT ROUND(exp.amount_original * p.completion_exchange_rate, 2)
      FROM financial_transactions exp
      WHERE exp.proposal_id = p.id
        AND exp.direction   = 'expense'
        AND exp.deleted_at  IS NULL
      ORDER BY exp.created_at
      LIMIT 1
    )
    ELSE ft.cogs_try
  END,
...
WHERE ft.proposal_id = p.id
  ...
  AND ft.income_type = 'sale'
  AND ft.original_currency = 'USD'
  AND p.status = 'completed'
  AND UPPER(COALESCE(p.currency, 'USD')) = 'USD'
  AND p.completion_exchange_rate IS NOT NULL
  AND p.completion_exchange_rate > 0
  AND (
    ft.exchange_rate IS NULL
    OR ABS(ft.exchange_rate - p.completion_exchange_rate) > 0.0001
  );
...
SET
  ...
  amount_try    = ROUND(ft.amount_original * p.completion_exchange_rate, 2),
  input_vat     = CASE
    WHEN ft.amount_original > 0 THEN ROUND(
      ROUND(ft.amount_original * p.completion_exchange_rate, 2) * COALESCE(p.vat_rate, 20) / 100,
      2
    )
    ELSE NULL
