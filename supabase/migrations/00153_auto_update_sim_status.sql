-- Migration: 00153_auto_update_sim_status
-- Description: Phase 4 G7 — Ensure SIM status auto-updates when subscription is
--   cancelled or paused. The trigger fn_subscription_sim_status_on_update already
--   exists (00057) and handles this; we reinforce it and ensure the trigger is
--   attached. Fires on: fn_cancel_subscription RPC (UPDATE) and pauseSubscription
--   direct UPDATE. When status -> cancelled/paused, linked SIM -> 'available'.

-- Recreate the function to ensure correct logic (idempotent)
CREATE OR REPLACE FUNCTION fn_subscription_sim_status_on_update()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  -- 1. Handle status changes: cancelled/paused -> SIM available; reactivate -> subscription
  IF NEW.sim_card_id IS NOT NULL THEN
    IF NEW.status IN ('cancelled', 'paused') AND (OLD.status IS NULL OR OLD.status NOT IN ('cancelled', 'paused')) THEN
      UPDATE sim_cards
      SET status = 'available', customer_id = NULL, site_id = NULL
      WHERE id = NEW.sim_card_id;
    ELSIF NEW.status = 'active' AND OLD.status IN ('cancelled', 'paused') THEN
      UPDATE sim_cards
      SET status = 'subscription',
          customer_id = (SELECT customer_id FROM customer_sites WHERE id = NEW.site_id),
          site_id = NEW.site_id
      WHERE id = NEW.sim_card_id;
    END IF;
  END IF;

  -- 2. Handle sim_card_id added or changed (subscription active/paused)
  IF NEW.status IN ('active', 'paused') THEN
    IF NEW.sim_card_id IS NOT NULL AND (OLD.sim_card_id IS NULL OR OLD.sim_card_id <> NEW.sim_card_id) THEN
      UPDATE sim_cards
      SET status = 'subscription',
          customer_id = (SELECT customer_id FROM customer_sites WHERE id = NEW.site_id),
          site_id = NEW.site_id
      WHERE id = NEW.sim_card_id;
      IF OLD.sim_card_id IS NOT NULL AND OLD.sim_card_id <> NEW.sim_card_id THEN
        UPDATE sim_cards
        SET status = 'available', customer_id = NULL, site_id = NULL
        WHERE id = OLD.sim_card_id;
      END IF;
    ELSIF NEW.sim_card_id IS NULL AND OLD.sim_card_id IS NOT NULL THEN
      UPDATE sim_cards
      SET status = 'available', customer_id = NULL, site_id = NULL
      WHERE id = OLD.sim_card_id;
    END IF;
  END IF;

  RETURN NEW;
END;
$$;

-- Ensure trigger exists (idempotent)
DROP TRIGGER IF EXISTS trg_subscription_sim_update ON subscriptions;
CREATE TRIGGER trg_subscription_sim_update
  AFTER UPDATE ON subscriptions
  FOR EACH ROW EXECUTE FUNCTION fn_subscription_sim_status_on_update();
