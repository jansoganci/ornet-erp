-- Migration: 00060_sim_card_status_migrate_cancelled
-- Description: Migrate inactive/sold -> cancelled, update view and trigger.
-- Depends on 00059 (cancelled must exist in enum).

-- ============================================================================
-- 1. Migrate data: inactive, sold -> cancelled
-- ============================================================================

UPDATE sim_cards SET status = 'cancelled' WHERE status IN ('inactive', 'sold');

UPDATE sim_card_history
SET old_status = 'cancelled'
WHERE old_status IN ('inactive', 'sold');

UPDATE sim_card_history
SET new_status = 'cancelled'
WHERE new_status IN ('inactive', 'sold');

-- ============================================================================
-- 2. Update view_sim_card_stats: inactive_count, sold_count -> cancelled_count
-- ============================================================================
...
-- ============================================================================
-- 3. Update fn_sim_card_to_finance: process 'cancelled' instead of 'inactive'
-- ============================================================================
...
BEGIN
  ...
  -- Only process status = active or cancelled (expense for both; income only for active)
  ...
  -- 1. EXPENSE (cost_price) — always for active and cancelled
  ...
  -- 2. INCOME (sale_price) — only for active, wholesale
