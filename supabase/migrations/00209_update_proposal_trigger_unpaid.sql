-- 00209_update_proposal_trigger_unpaid.sql
--
-- Proposal revenue rows now default to payment_status = 'unpaid' so they
-- appear on /finance/receivables for the Finance Team to track and collect.
--
-- Accrual accounting: revenue is recognized on completion (period/P&L), but
-- cash collection is tracked separately via financial_transaction_payments.
--
-- Only the two INCOME inserts are modified. COGS expense inserts are untouched
-- (they default to payment_status = 'paid' which is correct for internal costs).
