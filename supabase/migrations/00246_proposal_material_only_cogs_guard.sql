-- 00246_proposal_material_only_cogs_guard.sql
--
-- Corrective migration:
-- Proposal ledger COGS must come only from revenue_type = 'material' rows.
--
-- Scope:
-- 1. Recreate the active auto_record_proposal_revenue() function only.
-- 2. Keep proposal revenue posting unchanged.
-- 3. Prevent labor_service / other rows from contributing to cogs_try or mirrored material expense.
-- 4. Keep top-level proposals.cost_usd fallback only as a guarded legacy path for material-only proposals.
--
-- Out of scope:
-- - historical cleanup
-- - frontend changes
...
DECLARE
  ...
  v_has_material_rows   BOOLEAN := false;
  v_has_non_material_rows BOOLEAN := false;
...
BEGIN
  ...
  IF v_currency = 'TRY' THEN
    ...
    FOR v_item IN
      SELECT pi.quantity, pi.revenue_type, pi.cost, pi.product_cost, pi.material_cost
      FROM proposal_items pi
      WHERE pi.proposal_id = NEW.id
    LOOP
      IF v_item.revenue_type = 'material' THEN
        v_has_material_rows := true;
        ...
        ELSE
          -- Legacy generic fallback is allowed only for material rows.
          v_item_cogs := COALESCE(v_item.cost, 0);
        END IF;
      ELSE
        -- labor_service / other are revenue-only classifications for ledger purposes.
        v_has_non_material_rows := true;
        v_item_cogs := 0;
      END IF;
    ...
    -- Guarded legacy fallback: only for material-only proposals with no labor_service/other rows.
    IF v_cogs_total_try = 0 AND v_has_material_rows AND NOT v_has_non_material_rows THEN
  ...
  FOR v_item IN
    SELECT pi.quantity, pi.revenue_type, pi.cost_usd, pi.product_cost_usd, pi.material_cost_usd
    FROM proposal_items pi
    WHERE pi.proposal_id = NEW.id
  LOOP
    IF v_item.revenue_type = 'material' THEN
  ...
  -- Guarded legacy fallback: only for material-only proposals with no labor_service/other rows.
  IF v_cogs_usd = 0 AND v_has_material_rows AND NOT v_has_non_material_rows THEN
