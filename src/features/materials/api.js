 ...
 */
export const MATERIAL_LIST_SELECT = 'id, code, name, description, unit, category, is_active, created_at, unit_price, cost_price, currency';
...
export async function fetchMaterials(filters = {}) {
  ...
  if (filters.search) {
    ...
    query = query.or(`name_search.ilike.%${normalized}%,description_search.ilike.%${normalized}%`);
  }
  if (filters.dateFrom) {
