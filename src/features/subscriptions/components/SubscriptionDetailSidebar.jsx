...
export function SubscriptionDetailSidebar({ subscription, isAdmin, simCardId }) {
  ...
  return (
    <aside className="flex min-w-0 flex-col gap-4 lg:col-span-1">
      ...
      <Card className={cn(SURFACE, 'p-4 sm:p-5')}>
        ...
        <div className="space-y-0.5">
          {subscription.service_type && (
            <MetaRow
              label={t('subscriptions:detail.fields.serviceType')}
              value={t(`subscriptions:serviceTypes.${subscription.service_type}`)}
            />
          )}
          <MetaRow
      ...
      {simCardId && (
        <StaticIpCard simCardId={simCardId} isAdmin={isAdmin} className={SURFACE} compact />
      )}
    </aside>
  );
}
